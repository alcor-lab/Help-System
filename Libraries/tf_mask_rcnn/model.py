    ...
    def call(self, inputs):
        with tf.variable_scope("ProposalLayer", reuse=tf.AUTO_REUSE):
            ...
            proposals = utils.batch_slice([boxes, scores], nms,
                                          self.config.IMAGES_PER_GPU)
            return proposals
        #output must be (None, 1000, 4) or at least (None, None, 4)

    def compute_output_shape(self, input_shape):
        return (None, self.proposal_count, 4)

class MyProposalLayer:
    """Receives anchor scores and selects a subset to pass as proposals
    to the second stage. Filtering is done based on anchor scores and
    non-max suppression to remove overlaps. It also applies bounding
    box refinement deltas to anchors.
    ...
    def run(self, inputs):
        with tf.variable_scope("ProposalLayer", reuse=tf.AUTO_REUSE):
            ...
            scores = tf.map_fn(lambda x: tf.gather(x[0],x[1]), (scores, ix), dtype=tf.float32)
            deltas = tf.map_fn(lambda x: tf.gather(x[0], x[1]), (deltas, ix), dtype=tf.float32)
            pre_nms_anchors = tf.map_fn(lambda x: tf.gather(x[0],x[1]), (anchors, ix), dtype=tf.float32, name="pre_nms_anchors")
            # Apply deltas to anchors to get refined anchors.
            # [batch, N, (y1, x1, y2, x2)]
            boxes = tf.map_fn(lambda x: apply_box_deltas_graph(x[0], x[1]), (pre_nms_anchors, deltas), dtype=tf.float32, name="refined_anchors")

            # Clip to image boundaries. Since we're in normalized coordinates,
            # clip to 0..1 range. [batch, N, (y1, x1, y2, x2)]
            window = np.array([0, 0, 1, 1], dtype=np.float32)
            boxes = tf.map_fn(lambda x: clip_boxes_graph(x, window), (boxes), dtype=tf.float32, name="refined_anchors_clipped")
            ...
            proposals = tf.map_fn(lambda x: nms(x[0], x[1]), (boxes, scores), dtype=tf.float32)
            return proposals
        #output must be (None, 1000, 4) or at least (None, None, 4)
...
class MyDetectionTargetLayer():
    ...
    def graph(self, inputs):
        with tf.variable_scope("MyDetectionTargetLayer", reuse=tf.AUTO_REUSE):
            proposals = inputs[0]
            gt_class_ids = inputs[1]
            gt_boxes = inputs[2]
            gt_masks = inputs[3]
            ...
            # TODO: Rename target_bbox to target_deltas for clarity
            names = ["rois", "target_class_ids", "target_bbox", "target_mask"]
            #dtgc = detection_targets_graph_class(self.config)
            #outputs = tf.map_fn(lambda x: dtgc.run(x[0], x[1], x[2], x[3]),(proposals, gt_class_ids, gt_boxes, gt_masks), dtype=tf.float32)
            outputs = tf.map_fn(lambda x: detection_targets_graph(x[0], x[1], x[2], x[3], self.config),(proposals, gt_class_ids, gt_boxes, gt_masks), dtype=tf.float32)
...
class MyDetectionLayer():
    ...
    def __init__(self, mode, config=None, **kwargs):
        ...
        self.mode = mode
    def graph(self, inputs):
        with tf.variable_scope("MyDetectionLayer", reuse=tf.AUTO_REUSE):
            rois = inputs[0]
            mrcnn_class = inputs[1]
            mrcnn_bbox = inputs[2]
            image_meta = inputs[3]
            ...
            # Run detection refinement graph on each item in the batch
            detections_batch = tf.map_fn(lambda x: refine_detections_graph(x[0], x[1], x[2], x[3], self.config, self.mode), (rois, mrcnn_class, mrcnn_bbox, window), dtype=tf.float32)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    with tf.variable_scope("fpn_classifier_graph", reuse=tf.AUTO_REUSE):
        ...
        MyPROIA = MyPyramidROIAlign([pool_size, pool_size], name="roi_align_classifier")
        x = MyPROIA.myRun([rois, image_meta] + feature_maps)
        ...
        #x = tf.keras.layers.TimeDistributed(tf.layers.conv2d(x, fc_layers_size, (pool_size, pool_size), padding="valid"), name="mrcnn_class_conv1")
        ...
        x = tf.map_fn(lambda inputX: tf.layers.conv2d(inputX, fc_layers_size, (pool_size, pool_size), padding="valid"), x, name="mrcnn_class_conv1")
        #x = tf.keras.layers.TimeDistributed(tf.layers.conv2d(x, fc_layers_size, (pool_size, pool_size), padding="valid"))(x)
        x = tf.map_fn(lambda inputX: tf.layers.batch_normalization(inputX, training=train_bn), x, name="mrcnn_class_bn1")
        x = tf.nn.relu(x)
        x = tf.map_fn(lambda inputX: tf.layers.conv2d(inputX, fc_layers_size, (1, 1)), x, name="mrcnn_class_conv2")
        x = tf.map_fn(lambda inputX: tf.layers.batch_normalization(inputX, training=train_bn), x, name="mrcnn_class_bn2")
        x = tf.nn.relu(x)
        ...
        # Classifier head
        #shared = tf.transpose(shared, (1, 0, 2))
        mrcnn_class_logits = tf.map_fn(lambda inputX: tf.layers.dense(inputX, int(num_classes)), shared, name="mrcnn_class_logits")

        mrcnn_probs = tf.map_fn(lambda inputX: tf.nn.softmax(inputX), mrcnn_class_logits)
        ...
        x = tf.map_fn(lambda inputX: tf.layers.dense(inputX, int(num_classes*4), activation='linear'), shared, name='mrcnn_bbox_fc')
        ...
        #mrcnn_bbox = tf.transpose(mrcnn_bbox,  perm=[1,0,2,3])
        #mrcnn_class_logits = tf.transpose(mrcnn_class_logits, perm=[1,0,2])
        #mrcnn_probs = tf.transpose(mrcnn_probs, perm=[1,0,2])
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    with tf.variable_scope("build_fpn_mask_graph", reuse=tf.AUTO_REUSE):
        MyPROIA = MyPyramidROIAlign([pool_size, pool_size], name="roi_align_mask")
        x = MyPROIA.myRun([rois, image_meta] + feature_maps)

        x = tf.map_fn(lambda inputX: tf.layers.conv2d(inputX, 256, (3, 3), padding="same"), x, name="mrcnn_mask_conv1")
        x = tf.map_fn(lambda inputX: tf.layers.batch_normalization(inputX, training=train_bn), x, name="mrcnn_mask_bn1")
        x = tf.nn.relu(x)

        x = tf.map_fn(lambda inputX: tf.layers.conv2d(inputX, 256, (3, 3), padding="same"), x, name="mrcnn_mask_conv2")
        x = tf.map_fn(lambda inputX: tf.layers.batch_normalization(inputX, training=train_bn), x, name="mrcnn_mask_bn2")
        x = tf.nn.relu(x)

        x = tf.map_fn(lambda inputX: tf.layers.conv2d(inputX, 256, (3, 3), padding="same"), x, name="mrcnn_mask_conv3")
        x = tf.map_fn(lambda inputX: tf.layers.batch_normalization(inputX, training=train_bn), x, name="mrcnn_mask_bn3")
        x = tf.nn.relu(x)

        x = tf.map_fn(lambda inputX: tf.layers.conv2d(inputX, 256, (3, 3), padding="same"), x, name="mrcnn_mask_conv4")
        x = tf.map_fn(lambda inputX: tf.layers.batch_normalization(inputX, training=train_bn), x, name="mrcnn_mask_bn4")
        x = tf.nn.relu(x)

        x = tf.map_fn(lambda inputX: tf.layers.conv2d_transpose(inputX, 256, (2, 2), strides=2, activation="relu"), x, name="mrcnn_mask_deconv")
        x = tf.map_fn(lambda inputX: tf.layers.conv2d(inputX, num_classes, (1, 1), strides=1, activation="sigmoid"), x, name="mrcnn_mask")
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    with tf.variable_scope("mrcnn_class_loss_graph", reuse=tf.AUTO_REUSE):
        ...
        #return target_class_ids, pred_class_logits
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        with tf.variable_scope("MaskRCNN", reuse = tf.AUTO_REUSE):
            ...
            with tf.variable_scope('Pyramid', reuse=tf.AUTO_REUSE):
                ...
                #tf.image.resize_nearest_neighbor(P5, size=(P5_W, P5_H), name="fpn_p5upsampled")
            ...
            self.rpn_feature_maps = rpn_feature_maps
            ...
            if mode == "training":
                ...
                ##anchors_function= lambda x: tf.Variable(anchors, name='anchors')
                #anchors = anchors_function(input_image)
                #anchors = KL.Lambda(lambda x: tf.Variable(anchors), name="anchors")(input_image)
                anchors = tf.keras.layers.Lambda(lambda x: tf.Variable(anchors), name="anchors")(input_image)
                #Maybe just a tf varible directly (for loop?)
            else:
                anchors = input_anchors

            # RPN Model
            #rpn = build_rpn_model_class(config.RPN_ANCHOR_STRIDE, len(config.RPN_ANCHOR_RATIOS), config.TOP_DOWN_PYRAMID_SIZE)
            ...
            for index in range(3):
                ...
                #elem= tf.concat([lista], 2, name= output_names[index])
            ...
            myProposalLayerClass =  MyProposalLayer(
                proposal_count=proposal_count,
                nms_threshold=config.RPN_NMS_THRESHOLD,
                name="ROI",
                config=config)
            rpn_rois = myProposalLayerClass.run([rpn_class, rpn_bbox, anchors])

            self.rpn_rois_aux = rpn_rois
            ...
            if mode == "training":
                ...
                #    MyDetectionTargetLayer(config, name="proposal_targets")([
                #        target_rois, input_gt_class_ids, gt_boxes, input_gt_masks])

                MyDTL = MyDetectionTargetLayer(config, name="proposal_targets")
                targetVectorInput = [target_rois, input_gt_class_ids, gt_boxes, input_gt_masks]


                rois, target_class_ids, target_bbox, target_mask = MyDTL.graph(targetVectorInput)
                ...
                # Network Heads
                # Proposal classifier and BBox regrget_anchorsessor heads
                mrcnn_class_logits, mrcnn_class, mrcnn_bbox, problem =\
                    fpn_classifier_graph(rpn_rois, mrcnn_feature_maps, input_image_meta,
                                         config.POOL_SIZE, config.NUM_CLASSES,
                                         train_bn=config.TRAIN_BN,
                                         fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)
                self.mrcnn_bbox_aux = mrcnn_bbox
                ...
                MyDTL = MyDetectionLayer(mode, config, name="mrcnn_detection")

                targetVectorInput = [rpn_rois, mrcnn_class, mrcnn_bbox, input_image_meta]
                #targetVectorInput = [mrcnn_class, input_image_meta, rpn_rois, mrcnn_bbox]
                detections = MyDTL.graph(targetVectorInput)
                self.detections_aux = detections
                ...
                mrcnn_mask = build_fpn_mask_graph(detection_boxes, mrcnn_feature_maps,
                                                  ...
                                                  config.NUM_CLASSES,
                                                  train_bn=config.TRAIN_BN)
                self.mrcnn_mask_aux = mrcnn_mask
    ...
    def saveModel(self, modelName="maskGraph/coco.ckpt"):
    ...
    def loadModel(self, modelName="/home/edoardo/Desktop/Mask_Edo/mask_model/model.ckpt"):
    ...
    """
    def myDetect(self, images, verbose=0):
        start = time.time()
        ...
        with tf.Session() as s:
            ...
            saver.restore(s, "/home/alcor-lab/Desktop/km/mask-master/model/model.ckpt")
            #rpn_class_logits, rpn_class, rpn_bbox, mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask, rpn_rois =\
            ...
            #start_rpn = time.time()
            #rpn_test = s.run(self.rpn_rois_aux, feed_dict={i: d for i, d in zip(self.model[0], actual_input)})
            #print("rpn time is: ", time.time() - start_rpn)
            #print(rpn_test.shape)

            #start_bbox = time.time()
            #bbox_test = s.run(self.mrcnn_bbox_aux,feed_dict={i: d for i, d in zip(self.model[0], actual_input)})
            #print("bbox time is: ", time.time()-start_bbox)
            #print("bbox shape is",bbox_test.shape)
    ...
    def myDetect(self, images, verbose=0):
        ...
        image_shape = molded_images[0].shape
        # Anchors
        anchors_time = time.time()
        anchors = self.get_anchors(image_shape)
        print("anchors time is :",anchors_time-time.time())
        # Duplicate across the batch dimension because Keras requires it
        # TODO: can this be optimized to avoid duplicating the anchors?
        anchors = np.broadcast_to(anchors, (self.config.BATCH_SIZE,) + anchors.shape)
        ...
        with tf.Session() as s:
            ...
            s.run(tf.global_variables_initializer())
            saver.restore(s, "/home/edoardo/Desktop/Mask_Edo/mask_model/model.ckpt")
            #rpn_class_logits, rpn_class, rpn_bbox, mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask, rpn_rois =\
            print("loading time",time.time()-start)
    ...
    def edoDetect(self, images, verbose=0):
        start = time.time()
        saver = tf.train.Saver()
        s = tf.Session()
        s.run(tf.global_variables_initializer())
        ...
        for iii in range(100):
            ...
            detections, _, _, mrcnn_mask, _, _, _ = \
                s.run(self.model[1][:8], feed_dict={i: d for i, d in zip(self.model[0], actual_input)})
            print("loading time is", start_aux - start_all)
            print("detection time is",time.time() - start_aux)
            print("full loading and detection time is",time.time() - start_all)
        results = []
        return results
    ...
    def myTrain(self, train_dataset, val_dataset, learning_rate, epochs=150):
        ...
        with tf.name_scope('Loss_Trainig'):
            #outputs = s.run(self.model[1], feed_dict={i: d for i, d in zip(self.model[0], inputs)})
        ...
        saver = tf.train.Saver()
        self.epoch = epochs#max(self.epoch, epochs)
        print("Total Steps:", self.config.STEPS_PER_EPOCH)
        print("Total Epoches:", self.epoch,'\n')

        with tf.Session() as s:
            s.run(init_var)
            #saver.restore(s, tf.train.latest_checkpoint("models/"))
            ...
            for epoch_number in range(self.epoch):
                ...
                pbar_step.close()
                pbar_epoch.update(1)
                save_path = saver.save(s, "/home/alcor-lab/Desktop/km/mask-master/zhai/shape.ckpt")
            # self.saveModel()
            log_writer_train.close()
            log_writer_val.close()
            pbar_epoch.close()
            save_path = saver.save(s, "models/shape.ckpt")
        print("\nIl training ha avuto successo!!!\n")
